...
struct MoodTrackerSecondStateView: View {
    @State private var selectedSound: MoodSound = .distortion
    ...
    private var toneCards: some View {
        HStack(spacing: 10) {
            toneCard(title: "Distortion", icon: "Vector-7", active: selectedSound == .distortion, iconW: 42, iconH: 42, iconX: 37, iconY: 20)
            toneCard(title: "CLEAN TONE", icon: "Group-3", active: selectedSound == .cleanTone, iconW: 58, iconH: 58, iconX: 29, iconY: 11)
            toneCard(title: "BREAKDOWN", icon: "svgviewer-output (8) 1", active: selectedSound == .breakdown, iconW: 60, iconH: 60, iconX: 28, iconY: 10)
        }
    }
    ...
    private var feelingCard: some View {
        ZStack(alignment: .topLeading) {
            ...
            Text("Today - Distortion: project is fire, but I am low on energy.")
